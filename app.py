...
#chat{flex:1;overflow-y:auto;padding:20px}.msg{margin:10px 0;padding:12px 16px;border-radius:12px;max-width:85%;line-height:1.5}
...
<div id="chat"><div class="msg ai">I'm Titech AI by Timileyin Samson - smarter model active. Ask me anything or say "generate image of..."</div></div>
...
def chat():
    ...
    if "generate image" in low or "picture of" in low:
        prompt = low.replace("generate image of","").replace("generate image","").replace("a picture of","").replace("picture of","").strip() or "futuristic city"
        url = "https://image.pollinations.ai/prompt/" + urllib.parse.quote(prompt) + "?nologo=true&seed=" + os.urandom(2).hex()
        return jsonify({"reply": "Here is your image for " + prompt + ":", "image": url, "download_url": url})
