...
<img src='/logo.png' style='width:38px;height:38px;border-radius:10px;object-fit:cover;box-shadow:0 0 12px #3b82f6' onerror="this.style.display='none'">
TITECH AI
</div>
...
<script>let c=document.getElementById('chat');async function send(){let i=document.getElementById('inp'),t=i.value.trim();if(!t)return;c.innerHTML+=`<div style='align-self:flex-end;background:#2563eb;padding:12px 16px;border-radius:18px;max-width:80%'>${t}</div>`;i.value='';c.innerHTML+=`<div id=tmp style='align-self:flex-start;background:#161f3a;padding:12px;border-radius:18px'>Searching...</div>`;c.scrollTop=c.scrollHeight;let r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:t})});let d=await r.json();document.getElementById('tmp')?.remove();let imgHtml=d.image?`<div style='margin-top:10px;border-radius:12px;overflow:hidden;background:#0f172a'><img src="${d.image}" referrerpolicy="no-referrer" style='width:100%;max-height:500px;object-fit:contain;display:block' onerror="this.src='https://upload.wikimedia.org/wikipedia/commons/thumb/0/05/Coat_of_arms_of_Nigeria.svg/800px-Coat_of_arms_of_Nigeria.svg.png'"></div>`:'';c.innerHTML+=`<div style='align-self:flex-start;background:#161f3a;border:1px solid #243157;padding:12px;border-radius:18px;max-width:85%'>${d.reply}${imgHtml}<div style='font-size:10px;opacity:0.6;margin-top:6px;word-break:break-all'>${d.image||''}</div></div>`;c.scrollTop=c.scrollHeight}document.getElementById('inp').addEventListener('keydown',e=>{if(e.key==='Enter')send()});c.innerHTML+=`<div style='align-self:flex-start;background:#161f3a;padding:12px;border-radius:18px'>Welcome to Titech AI! 🚀</div>`;</script></body></html>"""
